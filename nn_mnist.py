...
#layer1
W1 = tf.Variable(tf.zeros([784, node_n]))
b1 = tf.Variable(tf.zeros([node_n]))
layer1 = tf.sigmoid(tf.matmul(X, W1) + b1)

#layer2
W2 = tf.Variable(tf.zeros([node_n, node_n]))
b2 = tf.Variable(tf.zeros([node_n]))
layer2 = tf.sigmoid(tf.matmul(layer1, W2) + b2)

#layer3
W3 = tf.Variable(tf.zeros([node_n,10]))
